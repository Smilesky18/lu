...
double* lu_gp_sparse_parallel(double *a, int *asub, int *xa, int n, int nzl, int nzu, int *perm_c, int *perm_r, int *asub_L, int *xa_L, int *asub_U, int *xa_U, int level, int *order_k_xa, int *order_k_asub)
{
  ...
  tt = order_k_xa[1] - order_k_xa[0];
//#pragma omp parallel for private(k, current_column, j, jj) //num_threads(32) 
	for ( zz = 0; zz < tt; zz++ )
	{
		k = order_k_asub[zz];
		...
		{
			  L[i] = xxx[asub_L[i]] / U[jj];
			  xx[asub_L[i]] = 0;
		}
	}
  for ( qq = 1; qq <= level; qq++ )
  {
	  rr = order_k_xa[qq];
	  ss = order_k_xa[qq+1];
	  tt = ss - rr;
//#pragma omp parallel for private(k, current_column, j, row_column, row, temp, i, U_diag) //num_threads(32) 
	  for ( zz = 0; zz < tt; zz++ )
	  {

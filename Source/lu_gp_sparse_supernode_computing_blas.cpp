...
double* lu_gp_sparse_supernode_computing_blas(double *a, int *asub, int *xa, int n, int nzl, int nzu, int *perm_c, int *perm_r, int *asub_L, int *xa_L, int *asub_U, int *xa_U, int *sn_record)
{
  int row;
  double *L, *U, *xx;
  int *row_index;
  ...
  double row_count;
  int pack, r, t, w, ww, qq, qqq, sss, p, ss;
  int row_else1, row_else2, row_if;
  double *dense_vec, sum;
  int row_sn_start, row_sn_end;
  double *diag, *lower_diag, *diag_matrix;
  int ptr = 0;
  int max_column_su = 0;
  FILE *file = fopen("00000000.txt", "w");
  double temp;
  ...
  for ( k = 0; k < n; k++ )
  {
    ...
    for ( j = 0; j < row_column; j+=pack )
    {
        ...
        {   
            ...
            //fprintf(file, "%d\n", row_number_sn);
            //if ( row_number_sn > 100 ) max_column_su++;
           ...
           // printf("row = %d\n", row_num);
            ...
            diag = (double *)malloc(sizeof(double) * row_number_sn);
            lower_diag = (double *)malloc(sizeof(double) * row_num);
            diag_matrix = (double *)malloc(sizeof(double) * row_number_sn*row_number_sn);
            ww = xa_L[row_sn_start+1] - row_num;
            
            for ( qqq = 0; qqq < row_number_sn; qqq++ )
            {
//                 diag[qqq] = xx[row_sn_start+qqq];
...
//                 row_else1 = row_index[j+qqq];
//                 ptr = 0;
//                 for ( sss = xa_L[row_else1]; sss < xa_L[row_else1+1]-row_num; sss++ )
//                 {
// //                     xx[asub_L[sss]] -=  xx[row_else1]*L[sss];
//                     diag_matrix[qqq*row_number_sn+qqq+ptr] = L[sss];
//                     ptr++;
//                 }
//             }
//             cblas_dtrsv(CblasColMajor, CblasLower, CblasNoTrans, CblasUnit, row_number_sn, diag_matrix, row_number_sn, diag, 1);
//             for ( qqq = 0; qqq < row_number_sn; qqq++ )
//             {
//                 xx[row_sn_start+qqq] = diag[qqq];
//             }
            ...
            for ( r = 0; r < row_number_sn; r++ )
            {
                ...
                for ( t = 0; t < row_num; t++ )
                {
                    dense_vec[t+row_num*r] = L[w+t];
                }
            }
            cblas_dgemv(CblasColMajor, CblasNoTrans, row_num, row_number_sn, 1.0, dense_vec, row_num, diag, 1, 0.0, lower_diag, 1);
            for ( qq = 0; qq < row_num; qq++ )
            {
                xx[asub_L[ww+qq]] -= lower_diag[qq];
            }
